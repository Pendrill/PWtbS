...
public class MoveCameraDialogueDO : MonoBehaviour {
	...
	void Update () {
		...
		if (moveToObject) {
			...
			if (!back) {
				...
				if (!theDropOff.isTextBoxActive && transform.position == theObject.transform.position + offsetPosition && !theObjectPickupManager.isActive) {
					//unzoom
					moveToObject = false;
					zoomOut = true;
					//objectZoom = false;
				...
				//Camera.main.transform.eulerAngles = Vector3.Lerp (-Camera.main.transform.eulerAngles, originalCameraRotation, time);
				if (!theDropOff.isTextBoxActive && transform.position == theObject.transform.position + offsetPositionBack && !theObjectPickupManager.isActive) {
					//unzoom
					moveToObject = false;
					zoomOut = true;
					//objectZoom = false;
			...
			//if (!theDropOff.isTextBoxActive && transform.position == theObject.transform.position + offsetPosition) {
			//unzoom
			//moveToObject = false;
			//zoomOut = true;
			//objectZoom = false;
